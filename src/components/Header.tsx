      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <button
            ...
            className="md:hidden p-2 rounded-lg hover:bg-gray-800/50 transition-colors text-gray-300 hover:text-cyan-400"
          >
            ...
            <nav className="flex flex-col space-y-2">
              {navItems.map((item) => (
                <button
                  key={item.name}
                  onClick={() => scrollToSection(item.href)}
                  className="text-left px-4 py-2 text-gray-300 hover:text-cyan-400 hover:bg-gray-800/30 rounded-lg transition-all duration-300"
                >
                  {item.name}
                </button>
              ))}
